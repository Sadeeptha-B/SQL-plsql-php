...
CREATE TABLE centre (
    ...
    centre_postcode  CHAR(10) NOT NULL,
...
CREATE TABLE class_staff (
    ...
    class_no            NUMBER(7) NOT NULL,
    staff_id            NUMBER(7) NOT NULL,
    staff_class_leader  CHAR(1) NOT NULL
);

ALTER TABLE class_staff
    ADD CONSTRAINT chk_class_leader CHECK ( staff_class_leader IN ( 'N', 'Y' ) );
...
COMMENT ON COLUMN class_staff.staff_class_leader IS
...
COMMENT ON COLUMN enrolment.enrollment_no IS
    'Enrollment Number';

COMMENT ON COLUMN enrolment.centre_id IS
    'Centre ID';

COMMENT ON COLUMN enrolment.class_no IS
    'Class number: unique within a centre';

COMMENT ON COLUMN enrolment.member_no IS
    'Member number';

COMMENT ON COLUMN enrolment.enroll_payment_date IS
    'Member''s Payment date for a class';

COMMENT ON COLUMN enrolment.enroll_total_attendance IS
    'The total attendance of the member for the enrollment';

ALTER TABLE enrolment ADD CONSTRAINT enrolment_pk PRIMARY KEY ( enrollment_no );
...
CREATE TABLE member_attendance (
    ...
    attend_session  CHAR(1) NOT NULL
);

ALTER TABLE member_attendance
    ADD CONSTRAINT chk_attended CHECK ( attend_session IN ( 'A', 'P' ) );

COMMENT ON COLUMN member_attendance.enrollment_no IS
    'Enrollment Number';

COMMENT ON COLUMN member_attendance.attend_date IS
    'Date of a class session';

COMMENT ON COLUMN member_attendance.attend_session IS
...
ALTER TABLE member_attendance
    ADD CONSTRAINT enroll_member_attend FOREIGN KEY ( enrollment_no )
        REFERENCES enrolment ( enrollment_no );
...
ALTER TABLE centre
    ADD CONSTRAINT staff_centre_manage FOREIGN KEY ( staff_id )
        REFERENCES staff_member ( staff_id );
